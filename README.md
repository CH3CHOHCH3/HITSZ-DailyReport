...
```
username password infohead
```

`infohead` 与上报内容有关。按照下方给出的方法获取。
...
当然，sh 脚本已经有了，定时运行的方式都可以自行安排。

## github action

> TODO：详细设置后续更新

开启 github action ，在 secrets 中加入 `username` 和 `password` `infohead` 三个 secrets。
